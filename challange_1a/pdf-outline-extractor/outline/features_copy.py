...
DATE_REGEX = re.compile(r"""
    (
        \d{1,2}[/-]\d{1,2}[/-]\d{2,4}         # 12/03/2023 or 12-03-2023
        |
        \d{4}[/-]\d{1,2}[/-]\d{1,2}           # 2023-03-12
        |
        \b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2},?\s+\d{2,4}  # Mar 3, 2023
    )
""", re.IGNORECASE | re.VERBOSE)

TIME_REGEX = re.compile(r'\b\d{1,2}:\d{2}(:\d{2})?\s*(AM|PM|am|pm)?\b')
...
HEADER_THRESHOLD = 50
FOOTER_THRESHOLD = 50
PAGE_HEIGHT = 842  # Default A4 height (adjust if needed)

def is_header_or_footer(line):
    y = line['y_top']
    return y < HEADER_THRESHOLD or y > (PAGE_HEIGHT - FOOTER_THRESHOLD)


def group_lines_by_page(lines):
...
def looks_like_toc_or_index(text):
    # Detect long leader dots
    if re.search(r'\.{5,}', text):
        return True
    # Detect multiple page numbers in one line
    if len(re.findall(r'\b\d{1,3}\b', text)) >= 2 and re.search(r'\b\d{1,3}\b\s+\b\d{1,3}\b', text):
        return True
    # Detect multiple section numbers (e.g. 3.1, 3.2)
    if len(re.findall(r'\b\d+\.\d+\b', text)) >= 2:
        return True
    
    if re.search(r'\b(?:Table|Figure|Appendix|Section|Chapter)\b', text, re.IGNORECASE):
        return True

    ONLY_NUMBERING_REGEX = re.compile(
    r"^(\s*(\d+(?:\.\d+)*|[IVXLC]+)\.?\s*){1,5}$",
    re.IGNORECASE)

    if ONLY_NUMBERING_REGEX.fullmatch(text.strip()):
        return True
...
def compute_stats(lines):
    ...
    sorted_sizes = [s for s, _ in size_counts.most_common()]  # descending by frequency
    body_size = sorted_sizes[0] if sorted_sizes else 0        # most common size
...
def detect_candidates(lines, stats):
    ...
    for line in lines:
        ...
        if line['y_top'] < 50 or line['y_top'] > (PAGE_HEIGHT - 50):
            continue
